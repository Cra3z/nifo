...
void main() {
    ...
    for (uint i = 0; i < parallel_lights.length(); ++i) {
        ...
        vec4 specular = material.specular_intensity * material.specular_color * texture(material.specular, gs_out.uv) * phong_spec(normal, parallel_lights[i].direction, eye, gs_out.position, material.shininess) * parallel_lights[i].specular * parallel_lights[i].color;
    ...
    for (uint i = 0; i < point_lights.length(); ++i) {
        ...
        vec4 specular = material.specular_intensity * material.specular_color * texture(material.specular, gs_out.uv) * phong_spec(normal, direction, eye, gs_out.position, material.shininess) * point_lights[i].specular * point_lights[i].color;
    ...
    for (uint i = 0; i < spot_lights.length(); ++i) {
        ...
        vec4 specular = material.specular_intensity * material.specular_color * texture(material.specular, gs_out.uv) * phong_spec(normal, light_direction, eye, gs_out.position, material.shininess) * spot_lights[i].specular * spot_lights[i].color;
