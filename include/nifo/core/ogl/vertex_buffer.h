...
namespace nifo {
	...
	public:
		...
		template<typename Rng> requires std::ranges::sized_range<Rng> && std::ranges::contiguous_range<Rng> && meta::plain<std::ranges::range_value_t<Rng>>
		basic_buffer(uint_t id_, const Rng& rng, buffer_usage usage) : basic_buffer(id_) {
			this->from_range(rng, usage);
		}

		template<typename... Rngs> requires (... && std::ranges::sized_range<Rngs>) && (... && std::ranges::contiguous_range<Rngs>) && (... && meta::plain<std::ranges::range_value_t<Rngs>>)
		...
		~basic_buffer() {
			nifo_gl_invoke(glDeleteBuffers, 1, &id);
		}
...
//		auto mmap(access_policy pol = access_policy::read | access_policy::write) ->void* {
//			return gl_invoke(glMapNamedBuffer, id, to_underlying(pol));
//		}
//
//		auto unmmap() ->bool {
//			return gl_invoke(glUnmapNamedBuffer, id);
//		}
//
//		auto mmapped() ->bool {
//			int_t mapped_;
...
//		}
//
//		auto mmap_address() ->void* {
//			void* addr;
//			nifo_gl_invoke(glGetNamedBufferPointerv, id, GL_BUFFER_MAP_POINTER, &addr);
//			return addr;
//		}
